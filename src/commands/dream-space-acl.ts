...
export function help() {
  return `
  Usage: ${chalk.bold(
    'dcl dream-space-acl [dream-space-name] SUBCOMMAND [options]'
  )}
    ...
    ${chalk.dim('Examples:')}
      ...
      ${chalk.green('$ dcl dream-space-acl name.dcl.eth show')}

      - Grant addresses 0x1 and 0x2 permission to deploy name.dcl.eth
      ${chalk.green('$ dcl dream-space-acl name.dcl.eth grant 0x1 0x2')}

      - Revoke addresses 0x1 and 0x2 permission to deploy name.dcl.eth
      ${chalk.green('$ dcl dream-space-acl name.dcl.eth revoke 0x1 0x2')}
`
}
...
async function fetchAcl(
  ...
  spinner.create(`Fetching acl for DreamSpace ${dreamSpaceName}`)
  try {
    const data = await fetch(`${targetContent}/acl/${dreamSpaceName}`)
      .then(checkStatus)
      .then((res) => res.json())
    spinner.succeed()
    return data
...
async function storeAcl(
  ...
  spinner.create(`Storing acl for DreamSpace ${dreamSpaceName}`)
  try {
    const data = await fetch(`${targetContent}/acl/${dreamSpaceName}`, {
      method: 'POST',
    ...
    spinner.succeed(`Stored acl for DreamSpace ${dreamSpaceName}`)
    return data
  } catch (error: any) {
    const message =
...
function displayPermissionToConsole(
  ...
  if (data.allowed.length === 0) {
    ...
    console.log(
      ...
      )} ${chalk.bold(dreamSpaceName)}${chalk.dim(':')}`
    )
...
async function showAcl(args: arg.Result<typeof spec>) {
  const dreamSpaceName = args._[1]
  const targetContent = args['--target-content']!

  try {
    const data = await fetchAcl(dreamSpaceName, targetContent)
    displayPermissionToConsole(data, dreamSpaceName)
  } catch (_) {
    process.exit(1)
  }
}
...
async function grantAcl(args: arg.Result<typeof spec>) {
  ...
  try {
    const currentData = await fetchAcl(dreamSpaceName, targetContent)
