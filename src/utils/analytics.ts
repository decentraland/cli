...
export namespace Analytics {
    ...
    try {
      analytics.identify({
        ...
        traits: {
          ...
          isCI: process.env.CI === "true" || (process.argv.includes("--ci") && process.argv.includes("--c")),
          devId,
        },
      })
    } catch (err: any) {
  ...
  export async function reportError(workingDir: string, type: string, message: string, stackTrace: string) {
    return track(
      "Error",
      {
        errorType: type,
