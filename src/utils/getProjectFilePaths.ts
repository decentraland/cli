...
export default async function getProjectFilePaths(dir: string, ignoreFileContent?: string): Promise<string[]> {
  ...
  const pResults = (await Promise.all(pendingPromises)).reduce((acc: string[], r: any) => {
    acc.push(...r)
    return acc
  }, [])
