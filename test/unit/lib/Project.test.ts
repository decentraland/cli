...
function createSandbox() {
  ...
  ctx.stub(fs, 'readFile').callsFake(async path => new Buffer('buffer'))
  ctx.stub(fs, 'stat').callsFake(async path => {
...
test('Unit - Project.getFiles() - should return all files', async t => {
  ...
  files.forEach((file, i) => {
    ...
    t.is(file.content.compare(new Buffer('buffer')), 0)
    t.is(file.size, 1000)
  })
})
...
test('Unit - Project.needsDependencies() - should return true if a package.json file is present', async t => {
  ...
  ctx.stub(fs, 'pathExists').callsFake(path => false)
